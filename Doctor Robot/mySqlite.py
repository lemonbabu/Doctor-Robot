...
def create_table():
    cursor.execute(
        " CREATE TABLE IF NOT EXISTS treatment_for_disease (plant TEXT, disease TEXT, treatment TEXT, audio_file TEXT)")


def data_entry(plant, disease, treatment, audio_file):
    cursor.execute("INSERT INTO treatment_for_disease (plant, disease, treatment, audio_file) VALUES (?, ?, ?, ?)",
                   (plant, disease, treatment, audio_file))
    connection.commit()


def read_from_db(plant, disease):
    cursor.execute(
        " SELECT audio_file, treatment FROM treatment_for_disease WHERE plant='" + plant + "' AND disease='" + disease + "'")
